...
const SingleTask: React.FC = () => {
  ...
  const fetchTaskData = async () => {
    ...
    try {
      const response = await getTaskById(
        `${ADMIN_URLS.GET_TASK_BY_ID}/${taskId}`
      );
      if (response.status === 200) {
        const task = response?.data?.data;

        const quizQuestions =
          task.taskType === "QUIZ" && task.quiz
            ? task.quiz.map((q) => ({
                title: q.question,
                _id: q._id,
        ...
        setFormData({
          ...
          file: null,
          answerType: task.answerType || "Select",
          checklistOptions:
            task.taskType === "CHECK_BOX"
              ? task.checkbox
                ? Object.values(task.checkbox)
                : []
              : ["Option 1", "Option 2", "Option 3", "Option 4"],
          link:
            Array.isArray(task.link) &&
            task.link.length > 0 &&
            task.link[0]?.trim() !== ""
              ? `${task.link[0]}`
              : "", // ✅ show empty input if API gave [] or [""]
          downloadFile: null,
          video: null,
          quizQuestions: quizQuestions,
          checkbox: task.checkbox || null,
          count: task.count || 0,
  ...
  const handleInputChange = (
    ...
    setFormData((prev) => ({ ...prev, 
     [name]: value.startsWith(BASE_IMG_URL) ? value.replace(BASE_IMG_URL, "") : value,
  ...
  const handleRemoveFile = (field: "file" | "downloadFile" | "video") => {
    ...
    setFileKey(null); // Clear uploaded file key
  };
  ...
  // Handle quiz question title change
  ...
  // Handle quiz correct option selection

  const handleQuizCorrectOptionChange = (questionIndex, optionIndex) => {
    setFormData((prev) => {
      const newQuestions = [...prev.quizQuestions];
      newQuestions[questionIndex] = {
        ...newQuestions[questionIndex],
  ...
  const removeQuizQuestion = async (index) => {
    const question = formData.quizQuestions[index];
    const payload = question._id;
    console.log(payload, "idididdidid");
    if (question._id) {
      // Only call API if the question has an _id (exists in backend)
      try {
        const response = await deleteQuiz(
          `${ADMIN_URLS.DELETE_QUIZ}/${question._id}`
        );
        if (response.status === 200) {
  ...
  const handleSaveQuiz = async () => {
    try {
      ...
      const response = await updateQuizTask(
  ...
  const updateTaskCheckbox = async (taskId, checkboxOptions) => {
    try {
  ...
  // Modified existing component code to include the save functionality
  const handleSaveChecklist = async () => {
    setLoading(true);
    try {
      await updateTaskCheckbox(taskId, formData.checklistOptions);
      // Optionally refetch task data to ensure UI is in sync
      await fetchTaskData();
    } catch (error) {
  ...
  const handleUpdateTask = async () => {
    ...
    try {
      ...
      if (fileToUpload) {
        uploadedKey = await uploadFile(fileToUpload);
      }
          ...
          <div className="w-full flex flex-col justify-center items-start gap-6 sm:gap-8 lg:gap-10">
            ...
            <div className="w-full flex flex-col justify-start items-start gap-4 sm:gap-5">
              ...
              <div className="w-full flex flex-col justify-start items-start gap-3 sm:gap-4">
                ...
                {formData.taskTypes.includes("Image") && (
                  <div className="w-full flex flex-col justify-start items-start gap-1.5">
                    <label className="text-stone-200 text-xs sm:text-sm font-light">
                      Upload Image
                    </label>
                    <input
                      ...
                      className="w-full p-3 sm:p-4 bg-zinc-900/80 rounded-[10px] outline outline-neutral-700 text-zinc-400 text-xs sm:text-sm font-light file:mr-2 file:py-1 file:px-2 file:rounded file:border-0 file:text-xs file:bg-zinc-700 file:text-zinc-300"
                    />
                  </div>
                )}
                ...
                {formData.taskTypes.includes("CHECK_BOX") && (
                  <div className="w-full flex flex-col justify-start items-start gap-1.5">
                    ...
                    <div className="w-full p-3 sm:p-4 rounded-[10px] outline outline-neutral-700 flex flex-col justify-start items-start gap-3 sm:gap-4 lg:gap-5">
                      ...
                      <div className="flex w-full flex-col sm:flex-row justify-between gap-2 sm:gap-0">
                        <button
                          onClick={addChecklistOption}
                          className="text-blue-500 text-sm sm:text-base font-normal leading-tight text-left"
                        >
                          + Add another Option
                        </button>
                        <button
                          ...
                          disabled={loading}
                          className={`text-blue-500 text-sm sm:text-base font-normal leading-tight text-left sm:text-right ${
                            loading
                ...
                {[
                  ...
                  <div className="w-full flex flex-col justify-start items-start gap-1.5">
                    ...
                    <input
                      ...
                      name="link"
                      value={
    formData.link?.startsWith("admin/")
      ? `${BASE_IMG_URL}${formData.link}`
      : formData.link
  }
                      onChange={handleInputChange}
                ...
                {formData.taskTypes.includes("Video") && (
                  <div className="w-full flex flex-col justify-start items-start gap-1.5">
                    <label className="text-stone-200 text-xs sm:text-sm font-light">
                      Upload Video
                    </label>
              ...
              <div className="w-full flex flex-col justify-start items-start gap-3 sm:gap-4">
                ...
                <div className="w-full flex flex-col justify-start items-start gap-1.5">
                  ...
                  <select
                    ...
                    disabled
                    aria-readonly
                    className="w-full px-3 sm:px-4 py-2.5 sm:py-3.5 bg-zinc-900/80 rounded-[10px] outline outline-neutral-700 text-zinc-400 text-xs sm:text-sm font-light cursor-not-allowed"
                  >
